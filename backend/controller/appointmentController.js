...
const createAppointment = async function (req, res) {
    let id_doctor = req.body.id_doctor;
    ...
    if (!id_doctor) {
        errors = true;
        res.json({ pesan: 'Field id_doctor belum diisi, mohon isi dengan lengkap.' });
    }
    ...
    if (!queue_no) {
        errors = true;
        res.json({ pesan: 'Field queue_no belum diisi, mohon isi dengan lengkap.' });
    }
    ...
    if (!rating) {
        errors = true;
        res.json({ pesan: 'Field rating belum diisi, mohon isi dengan lengkap.' });
    }

    if (!review) {
        errors = true;
        res.json({ pesan: 'Field review belum diisi, mohon isi dengan lengkap.' });
    }

    connection.query('SELECT * FROM tbl_doctors WHERE id_doctor = ?', [id_doctor], function (err, result) {
        if (err) {
            return res.status(500).json({ pesan: 'Terjadi kesalahan pada server saat memeriksa id_doctor', err });
        }
        if (result.length === 0) {
